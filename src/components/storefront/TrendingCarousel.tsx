      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {trendingProducts.map((product) => (
            <div
              key={product.id}
              className="w-[280px] sm:w-[320px] flex-shrink-0 group bg-neutral-900/60 border border-neutral-800 hover:border-neutral-600 rounded-lg overflow-hidden transition-all flex flex-col"
            >
              {/* Product Image Stage */}
              <div className="relative aspect-[4/5] bg-neutral-900 overflow-hidden cursor-pointer" onClick={() => setSelectedProduct(product)}>
                ...
                {/* Badge */}
                {product.badge && (
                  <span className="absolute top-3 left-3 px-2 py-0.5 bg-white text-black text-[10px] font-mono font-black uppercase rounded tracking-wider">
                    {product.badge}
                  </span>
                )}
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 p-4">
                  <button
                    ...
                    className="px-3 py-2 bg-neutral-950/90 text-white hover:bg-white hover:text-black text-xs font-mono rounded flex items-center gap-1.5 transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-cyan-400 text-black hover:bg-cyan-300 text-xs font-mono font-bold rounded flex items-center gap-1.5 transition-colors"
                  >
                    <Plus size={14} /> Add
                  </button>
                </div>
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <h3
                    ...
                    className="font-display font-bold text-sm text-white hover:text-cyan-400 transition-colors cursor-pointer line-clamp-1"
                  >
                    {product.name}
                  </h3>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-neutral-800/80 flex items-center justify-between">
                  <div>
                    <span className="text-base font-bold font-mono text-white">{formatINR(product.price)}</span>
                  </div>
